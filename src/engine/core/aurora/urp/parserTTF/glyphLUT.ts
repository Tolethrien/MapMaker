...
export function createGlyphLUT(
  ...
  return {
    ...
    kern: (firstCharacter: number, secondCharacter: number): number => {
      ...
      EngineDebugger.assertValue(firstGlyphID, {
        msg: `Glyph not found for: "${firstCharacter}"`,
      });
      EngineDebugger.assertValue(secondGlyphID, {
        msg: `Glyph not found for: "${secondCharacter}"`,
      });
...
function calculateGlyphQuads(ttf: TTF, alphabet?: string): Glyph[] {
  ...
  return charCodes.map((code) => {
    EngineDebugger.assertValue(ttf, { msg: "TTF is missing." });

    const index = ttf.cmap.glyphIndexMap.get(code);

    EngineDebugger.assertValue(index, {
      msg: `Couldn't find index for character '${String.fromCharCode(
        code
      )}' in glyphIndexMap.`,
    });
    EngineDebugger.assertValue(index < ttf.glyf.length, {
      msg: "Index is out of bounds for glyf table.",
    });

    const lastMetric = ttf.hmtx.hMetrics.at(-1);
    EngineDebugger.assertValue(lastMetric, {
      msg: "The last advance is missing, which means that hmtx table is probably empty.",
    });
  ...
  for (const box of rectangles) {
    for (let i = regions.length - 1; i >= 0; i--) {
      ...
      if (box.width === region.width && box.height === region.height) {
        ...
        EngineDebugger.assertValue(last, {
          msg: "Regions array should not be empty.",
        });
